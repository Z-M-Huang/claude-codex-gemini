...
4. Validate dependency claims by tracing imports and references
...
**Use the Write tool** to write the review output. The Gemini orchestrator specifies the output file path via the --output parameter when invoking this agent (e.g., .task/review-sonnet.json or .task/review-opus.json).
...
1. The review file has been written using the Write tool to the path provided in the instructions
...
The orchestrator will specify the output file path via --output parameter.
